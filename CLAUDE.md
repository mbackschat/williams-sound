...
- **UI/UX is high-priority, not a finishing touch.** Every user-facing surface (Explore, Designer, onboarding, manuals) is a *designed* thing — not "controls that happen to be on a page". Aim for:
  ...
  - **Every control with a keyboard shortcut ALWAYS shows that key in its hover tooltip** — discoverability is not optional. The binding's single source of truth is the Explore keymap (`web/ui/keymap.ts`); `web/ui/keyboard.ts` stamps each control's key onto a `data-shortcut` attribute and annotates its `title`. ⚠ A control whose `.title` is rewritten at runtime (the scrub Play/Freeze toggle, game-switcher buttons, …) **must** recompose through `setShortcutTitle(el, base)` (`web/ui/shortcutTitle.ts`) — assigning `.title` directly silently drops the `[Key]` tag.
